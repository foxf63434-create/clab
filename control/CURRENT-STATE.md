...
STATUS: HQ_WP04_R02_DEVELOPMENT_ACTIVE
...
CURRENT_CANDIDATE: `PENDING_NEW_R02_CANDIDATE`
CURRENT_CANDIDATE_TREE: `PENDING`.
CANDIDATE_DIFF_BASE: `b7445b57ae6db866cf4d75dc88e0626d4ff6c3e9`.
CANDIDATE_DIFF_STATUS: `PENDING_R02`.
CURRENT_CHANGED_BLOBS: `PENDING_R02`.
DEVELOPER_EVIDENCE: private `EVIDENCE/HQ-WP04-R02-DEVELOPER.md` — `PENDING`.
QA_EVIDENCE: private `EVIDENCE/HQ-WP04-R02-QA.md` — `NOT_STARTED`.
QA_TESTS: private `EVIDENCE/HQ-WP04-R02-QA-TESTS.py` — `NOT_STARTED`.
...
Before any Reviewer routing, a materially new R02 candidate must pass returned independent malformed-provenance regressions, the remaining R01 independent regressions, candidate-authored WP04 tests, and affected accepted WP01-WP03 provenance/identity/security regressions.
Network-backed `GitHubApiSource` remains explicitly `NOT_TESTED` unless actually exercised with reproducible evidence.
...
ASSIGNMENT_STATE: `TERMINAL_REJECT_WP04_R01 / DEPENDENCY_GATED_R02`
...
SCHEDULER_STATE: disabled after terminal R01 REJECT; must remain disabled until Registrar verifies and pins a materially new R02 Developer `READY_FOR_QA` candidate.
...
CURRENT_STATE: WP04 R01 ended in independent QA REJECT; R02 bounded correction is in Development and has no QA, Reviewer or Control PASS.
